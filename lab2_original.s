...
add		$t3, $t3, $t2			# M00 x X + M01 * y + M02 ===> $t3
lw		$t2, 12($a2)			# M10
mult	$t2, $t1					# M10 x X
mflo	$t4								# value move to $t3
lw		$t2, 16($a2)			# M11
mult	$t2, $t0					# M11 x y
mflo	$t5								# value move to $t4
lw		$t2, 20($a2)			# M12
add		$t4, $t4, $t5			# M10 x X + M11 * y
add		$t4, $t4, $t2			# M10 x X + M11 * y + M12 ---> $t4
...
mflo	$t5
add		$t5, $t5, $t1				# offset value of $a0 (input address) to $t5
multu	$t3, $a3						# to fine the address of $a1
mflo	$t6
add		$t6, $t6, $t4				# offset from $a1
add		$a0, $a0, $t5				# move the address
lbu		$t7, 0($a0)					# load byte stored in
sub		$a0, $a0, $t5				# going back to the original
add		$a1, $a1, $t4
sb		$t7, 0($a1)
sub		$a1, $a1, $t4				# going back to the original a1.
j			parentloop
