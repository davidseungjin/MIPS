...
andi	$t0, $0, 0x00			# initialize $t0 as 0 for looping. row x
andi	$t1, $0, 0x00			# initialize $t1 as 0 for nested looping. column y
...
nestedloop:
lbu		$t7, 0($a0)				# load byte at $t7 from $a0.
lw		$t2, 0($a2)				# M00
mult	$t2, $t0					# M00 x X
mflo	$t3								# value copy from lo to $t3
lw		$t2, 4($a2)				# M01
mult	$t2, $t1					# M01 * y
mflo	$t4								# value move to $t4
lw		$t2, 8($a2)				# M02
add		$t3, $t3, $t4			# M00 x X + M01 * y
add		$t3, $t3, $t2			# M00 x X + M01 * y + M02 ===> X0 -->  $t3
lw		$t2, 12($a2)			# M10
mult	$t2, $t0					# M10 x X
mflo	$t4								# value move to $t4
lw		$t2, 16($a2)			# M11
mult	$t2, $t1					# M11 x y
mflo	$t5								# value move to $t5
lw		$t2, 20($a2)			# M12
add		$t4, $t4, $t5			# M10 x X + M11 * y
add		$t4, $t4, $t2			# M10 x X + M11 * y + M12 ---> Y0 --> $t4
addi	$t1, $t1, 1				# $t1 increment by 1
j			comparison1

comparison1:
bgeu	$t3, $a3, makezero1
bgeu	$t4, $a3, makezero2
bltz	$t3, makezero1
bltz	$t4, makezero2
j			conversion

conversion:
multu	$t3, $a3						# to fine offset value of the address $a1. column * dimension
mflo	$t5
add		$t5, $t5, $t4				# offset value from $a1
add		$a1, $a1, $t5
sb		$t7, 0($a1)
sub		$a1, $a1, $t5				# going back to the original a1.
addi	$a0, $a0, 1					# move $a0 address for next loop.
j			parentloop

makezero1:
move	$t7, $0							# test
#and		$t7, $t7, $0					# value loaded from $a0 becomes zero
j			conversion
