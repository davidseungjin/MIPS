...
andi	$t0, $0, 0				# initialize $t0 as 0 for looping. x
andi	$t1, $0, 0				# initialize $t1 as 0 for nested looping. y
j			parentloop

parentloop:
blt		$t1, $a3, nestedloop
andi	$t1, $0, 0				# $t1 initialization for nested loop
addiu	$t0, $t0, 1				# $t0 increment by 1
bge		$t0, $a3, exitloop
j			parentloop
...
lw		$t2, 0($a2)				# M00
multu	$t2, $t0					# M00 x X
mflo	$t3								# copy the value M00 X from lo to $t3
lw		$t2, 4($a2)				# M01
multu	$t2, $t1					# M01 * y
mflo	$t4								# copy the value M01 Y from lo to $t4
lw		$t2, 8($a2)				# M02 at $t2
addu	$t3, $t3, $t4			# M00 X + M01 y at $t3
addu	$t3, $t3, $t2			# M00 X + M01 y + M02 ===> X0 stored at $t3
lw		$t2, 12($a2)			# M10 at $t2
multu	$t2, $t0					# M10 X
mflo	$t4								# copy the value M10 X from lo to $t4
lw		$t2, 16($a2)			# M11
multu	$t2, $t1					# M11 y
mflo	$t5								# copy the value M11 Y from lo to $t5
lw		$t2, 20($a2)			# M12 at $t2
addu	$t4, $t4, $t2			# M10 X + M11 y at $t4
addu	$t4, $t4, $t5			# M10 X + M11 y + M12 ---> Y0 stored at $t4
lbu		$t7, 0($a0)				# load byte data at $t7 from $a0.
j			comparison

comparison:							# if X0 or Y0 is out of condition, make output(x,y) zero.
bge		$t3, $a3, makevaluezero
bge		$t4, $a3, makevaluezero
bltz	$t3, makevaluezero
bltz	$t4, makevaluezero
j			conversion

makevaluezero:
andi	$t7, 0						# make $t7 value zero because $t3,$t4 out of range.
j			conversion

conversion:
multu	$t3, $a3					# to fine the address of $a1. x0 * dimension
mflo	$t5
addu	$t5, $t5, $t4			# X0 * dimension + Y0, offset value from $a1
addu	$a1, $a1, $t5			# new address to store byte or zero is offsetted from $a1
sb		$t7, 0($a1)
sub		$a1, $a1, $t5			# going back to the original address of $a1.
addiu	$a0, $a0, 1				# move to the next target address of $a0.
addiu	$t1, $t1, 1				# Y, $t1 increment by 1
j			parentloop
